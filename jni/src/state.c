...
#ifdef ANDROID
		memset( android_img_state_path, 0, 2048 );
		snprintf( android_img_state_path, 2048, "%s.bmp", st_name );
#endif
	}
	return gzf;
}
...
#ifdef ANDROID
	printf("saving screenshot to %s", android_img_state_path );
	SDL_SaveBMP( state_img, android_img_state_path );
	printf("done");
#endif
