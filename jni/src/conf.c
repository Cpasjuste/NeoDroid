...
#elif ANDROID
	cf_create_string_item("rompath", "Tell gngeo where your roms are", "PATH", 'i', getDataPath());
	cf_create_string_item("biospath", "Tell gngeo where your neogeo bios is", "PATH", 'B', getDataPath());
	char path[1024];
	snprintf(path, 1024, "%s/gngeo_data.zip", getDataPath() );
	cf_create_string_item("datafile", "Tell gngeo where his ressource file is", "PATH", 'd', path);
#else
...
#elif ANDROID
		int len = strlen("gngeorc") + strlen(getDataPath()) +  strlen("/conf/") + 1;
		conf_file = (char *) alloca(len * sizeof (char));
		snprintf( conf_file, len, "%s/conf/gngeorc", getDataPath() );
#else /* POSIX */
